#!/usr/bin/env python3
"""
Creation Date Cluster Mapper
Apply exact cluster assignments using creation date order alignment
"""
import pickle
import pandas as pd
import psycopg2
...
def main():
    """
    Apply exact cluster assignments using creation date order
    """
    print("📅 Creation Date Cluster Mapper")
    print("=" * 50)
    ...
    try:
        ...
        # Sort parquet data by creation date to match original ORION order
        print("📅 Sorting by creation date...")
        df_sorted = df.sort_values('Created').reset_index(drop=True)
        
        print(f"✅ Sorted {len(df_sorted)} forces by creation date")
        print(f"   Earliest: {df_sorted['Created'].iloc[0]}")
        print(f"   Latest: {df_sorted['Created'].iloc[-1]}")
        
        # Create title to cluster mapping using creation date order
        title_to_cluster = {}
        for i, (idx, row) in enumerate(df_sorted.iterrows()):
            if i < len(cluster_labels):
                title = row['Title']
                cluster_id = cluster_labels[i]
                if isinstance(title, str) and title.strip() and cluster_id is not None:
        ...
        if test_title in title_to_cluster:
            test_cluster = title_to_cluster[test_title]
            print(f"🎯 Test: '{test_title}' -> Cluster {test_cluster}")
        ...
        # Clear existing clusters (fix foreign key constraint)
        ...
        print(f"✅ Matched {matched_count} forces to {len(cluster_groups)} clusters")
        ...
        for cluster_id, force_ids in sorted(cluster_groups.items()):
            ...
            cursor.execute("""
                ...
                RETURNING id
            """, (cluster_name, force_ids, project_id, 'exact_creation_date', cluster_size))
        ...
        # Verify the problematic case
        cursor.execute("SELECT title, cluster_label FROM driving_forces WHERE title = %s", (test_title,))
        test_result = cursor.fetchone()
        if test_result:
            print(f"🎯 Verification: '{test_result[0]}' -> {test_result[1]}")
