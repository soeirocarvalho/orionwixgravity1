...
import { z } from "zod";
var projectTypeEnum = pgEnum("project_type", ["full_orion", "megatrends", "early_warning", "new_project"]);
var subscriptionStatusEnum = pgEnum("subscription_status", ["active", "canceled", "past_due", "unpaid", "trialing", "incomplete", "incomplete_expired"]);
var subscriptionTierEnum = pgEnum("subscription_tier", ["basic", "professional", "enterprise"]);
...
var drivingForces = pgTable("driving_forces", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
var clusteringReports = pgTable("clustering_reports", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id),
  projectId: varchar("project_id").references(() => projects.id).notNull(),
...
var aiUsageTracking = pgTable("ai_usage_tracking", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id).notNull(),
...
var inviteCodesRelations = relations(inviteCodes, ({ one }) => ({
  createdBy: one(users, {
...
var drivingForcesRelations = relations(drivingForces, ({ one }) => ({
  project: one(projects, {
    fields: [drivingForces.projectId],
    references: [projects.id]
  })
}));
var clustersRelations = relations(clusters, ({ one }) => ({
  project: one(projects, {
    fields: [clusters.projectId],
    references: [projects.id]
  })
}));
var workspacesRelations = relations(workspaces, ({ one }) => ({
  project: one(projects, {
    fields: [workspaces.projectId],
    references: [projects.id]
  })
}));
var clusteringReportsRelations = relations(clusteringReports, ({ one }) => ({
  project: one(projects, {
...
var subscriptionHistoryRelations = relations(subscriptionHistory, ({ one }) => ({
  user: one(users, {
    fields: [subscriptionHistory.userId],
    references: [users.id]
  })
}));
var aiUsageTrackingRelations = relations(aiUsageTracking, ({ one }) => ({
  user: one(users, {
    fields: [aiUsageTracking.userId],
    references: [users.id]
  })
}));
var projectTypeSchema = z.enum(["full_orion", "megatrends", "early_warning", "new_project"]);
...
var insertDrivingForceSchema = createInsertSchema(drivingForces).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});
var updateDrivingForceSchema = createInsertSchema(drivingForces).omit({
  id: true,
  createdAt: true,
  updatedAt: true
}).partial();
var insertClusterSchema = createInsertSchema(clusters).omit({
  id: true,
  createdAt: true
});
...
var insertClusteringReportSchema = createInsertSchema(clusteringReports).omit({
  id: true,
  createdAt: true
});
...
var insertSubscriptionHistorySchema = createInsertSchema(subscriptionHistory).omit({
  id: true,
  createdAt: true
});
var insertAiUsageTrackingSchema = createInsertSchema(aiUsageTracking).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});
...
var passwordResetRequestSchema = z.object({
  email: z.string().email("Please enter a valid email address")
});
...
var savedSearchesRelations = relations(savedSearches, ({ one }) => ({
  project: one(projects, {
...
var facetCountsSchema = z.object({
...
var bulkEditFieldsSchema = z.object({
...
var bulkEditFiltersSchema = z.object({
  forceIds: z.array(z.string()).optional(),
...
var searchFiltersSchema = searchQuerySchema.omit({
  page: true,
  pageSize: true,
  sort: true,
  sortOrder: true,
...
var chatStreamRequestSchema = z.object({
...
var importStatusResponseSchema = z.object({
  success: z.boolean(),
  data: z.object({
...
var importClustersResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
  data: z.object({
...
var adminErrorResponseSchema = z.object({
  success: z.literal(false),
  error: z.string(),
  hint: z.string().optional()
});
var apiSuccessResponseSchema = (dataSchema) => z.object({
...
var subscriptionStatusResponseSchema = z.object({
  ...
  async createProject(project) {
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        const existingProjects = await tx.select().from(projects).where(sql2`LOWER(name) = LOWER(${projectData.name})`);
  ...
  async generateReport(reportId, projectId, format) {
    try {
      ...
      if (report.reportType === "chat" && report.chatHistory) {
        ...
        const clusters2 = await storage.getClusters(projectId);
        await new Promise((resolve) => setTimeout(resolve, 2e3));
        reportBuffer = Buffer.from(`Standard Report for ${project.name}
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/v1/analytics/radar", authenticateToken, async (req, res) => {
    try {
      ...
      console.error("Failed to get radar data:", error);
...
var DatabaseSeeder = class {
  ...
  async seedIfNeeded() {
    try {
      ...
      const defaultProject = await this.ensureDefaultProject();
      const [forceCount] = await db.select({ count: sql3`count(*)` }).from(drivingForces).where(eq2(drivingForces.projectId, defaultProject.id));
      ...
      if (count2 > 0 && count2 < expectedCount) {
        ...
        await db.delete(drivingForces).where(eq2(drivingForces.projectId, defaultProject.id));
   ...
   */
  async ensureDefaultProject() {
    const [existingDefault] = await db.select().from(projects).where(eq2(projects.isDefault, true)).limit(1);
    if (existingDefault) {
      return existingDefault;
    }
  ...
  async importSeedData(projectId) {
    ...
    const [finalCount] = await db.select({ count: sql3`count(*)` }).from(drivingForces).where(eq2(drivingForces.projectId, projectId));
    ...
    const typeStats = await db.select({
      type: drivingForces.type,
      count: sql3`count(*)`
    }).from(drivingForces).where(eq2(drivingForces.projectId, projectId)).groupBy(drivingForces.type);
