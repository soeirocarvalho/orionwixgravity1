    ...
    try {
      ...
      if (report.reportType === 'chat' && report.chatHistory) {
        ...
        if (report.selectedForceIds && Array.isArray(report.selectedForceIds) && report.selectedForceIds.length > 0) {
          reportForces = forces.forces.filter(f => (report.selectedForceIds as string[]).includes(f.id));
        }
        ...
        reportBuffer = await pdfService.generateStandardReport({
          ...
          forces: reportForces,
          selectedForceIds: Array.isArray(report.selectedForceIds) ? report.selectedForceIds : undefined,
          sections: Array.isArray(report.sections) ? report.sections : undefined
        });
